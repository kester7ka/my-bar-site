...
// --- Проверка входа только через Telegram (можно временно закомментировать для тестов) ---
if (!userId) {
...
function showMenu() {
  setPageTitle('');
  showPage(`
    <div class="welcome-block" style="margin-bottom: 10px;">
      <div class="welcome-greet" style="margin-bottom: 0;">${escapeHtml(welcomeGreeting())}, ${USER ? escapeHtml(USER.username) : ""}! <span class='greet-emoji'>👋</span></div>
    </div>
    <div class="expired-main-block custom-expired-block">
      <div class="expired-icon-center">
        <span class="expired-icon-svg">
          <svg width="38" height="38" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="19" cy="19" r="19" fill="#232b33"/>
            <path d="M19 10V22" stroke="#FFD369" stroke-width="2.5" stroke-linecap="round"/>
            <circle cx="19" cy="27" r="1.7" fill="#FFD369"/>
          </svg>
        </span>
      </div>
      <div class="expired-title-text" style="text-align:center; margin-bottom: 10px; font-size:1.18em; font-weight:700; color:#fff;">Некондиционные позиции</div>
      <div class="expired-filters-center">
      ...
      <div id="mainExpiredTitle" style="text-align:center;color:#fff;font-size:1.07em; margin-top:10px;">Загрузка...</div>
      <div id="mainExpiredCards"></div>
    </div>
    <div id="mainContentAfterExpired"></div>
  `);

  // Анимация панели навигации только после загрузки (один раз)
  if (!window._navAnimatedOnce) {
...
function showSearchPage() {
  ...
  showPage(addBackButton(`
    <div id="searchBlock" class="beautiful-form" style="gap:10px;max-width:440px;">
      ...
      <div class="search-dropdown-row" style="display:flex;gap:12px;align-items:center;margin-bottom:10px;">
        <select id="categoryDropdown" class="search-dropdown" style="flex:1;">
        ...
        <select id="statusDropdown" class="search-dropdown" style="width:140px;">
