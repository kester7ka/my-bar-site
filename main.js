...
// Подключаем qrious через CDN (добавить в index.html):
// <script src="https://cdn.jsdelivr.net/npm/qrious@4.0.2/dist/qrious.min.js"></script>

function showProfilePage() {
  setPageTitle('Профиль');
  showBottomNav(true);
  showPage('<div id="profileBlock" class="beautiful-form" style="max-width:420px;text-align:center;">Загрузка...</div>');
  ...
  .then(data => {
    if (!data.ok) {
      document.getElementById('profileBlock').innerHTML = `<div class="error">Ошибка: ${escapeHtml(data.error)}</div>`;
      return;
    }
    let { username, bar_name, qr_data } = data;
    let userPhoto = tg && tg.initDataUnsafe && tg.initDataUnsafe.user && tg.initDataUnsafe.user.photo_url ? tg.initDataUnsafe.user.photo_url : '';
    let html = '';
    html += `<div style="display:flex;flex-direction:column;align-items:center;gap:18px;">`;
    html += `<img src="${escapeHtml(userPhoto)}" class="user-avatar" style="width:84px;height:84px;margin:0 auto 0 auto;box-shadow:0 2px 18px #7b7bff55,0 2px 1.5px #232b33 inset;border:3px solid #7b7bff;">`;
    html += `<div style="font-size:1.35em;font-weight:800;color:#fff;">${escapeHtml(username||'')}</div>`;
    html += `<div style="font-size:1.08em;color:#7b7bff;font-weight:700;margin-bottom:8px;">${escapeHtml(bar_name||'')}</div>`;
    if (!qr_data) {
      html += `<form id="qrUploadForm" enctype="multipart/form-data" style="margin:18px 0 0 0;display:flex;flex-direction:column;align-items:center;gap:12px;">
        <label style="color:#b9dbff;font-weight:700;">Загрузите фото вашего QR-кода</label>
        <input type="file" name="file" id="qrFileInput" accept="image/*" style="padding:10px 0;" required>
        <button type="submit" style="padding:12px 32px;border-radius:13px;background:#7b7bff;color:#fff;font-weight:700;border:none;box-shadow:0 2px 12px #7b7bff33;">Загрузить</button>
      </form>`;
      html += `<div id="qrUploadMsg" style="margin-top:12px;"></div>`;
    } else {
      html += `<div style="margin:18px 0 0 0;display:flex;flex-direction:column;align-items:center;gap:10px;">
        <label style="color:#b9dbff;font-weight:700;">Ваш QR-код</label>
        <canvas id="userQrCanvas" width="180" height="180" style="margin:0 auto 0 auto;border-radius:18px;box-shadow:0 2px 18px #7b7bff33;"></canvas>
        <button id="reuploadQrBtn" style="margin-top:10px;padding:10px 24px;border-radius:13px;background:#232b33;color:#7b7bff;font-weight:700;border:none;box-shadow:0 2px 12px #7b7bff11;">Загрузить новый QR-код</button>
      </div>`;
    }
    html += `<div style="margin-top:28px;color:#888;font-size:1.05em;">Версия приложения: <b>v0.7</b></div>`;
    html += `</div>`;
    document.getElementById('profileBlock').innerHTML = html;
    // Генерируем QR, если есть
    if (qr_data) {
      if (window.QRious) {
        new QRious({
          element: document.getElementById('userQrCanvas'),
          value: qr_data,
          size: 180,
          background: '#fff',
          foreground: '#232b33',
        ...
        document.getElementById('userQrCanvas').replaceWith('<div style="color:#ff6b81;">QR-библиотека не подключена</div>');
      }
      document.getElementById('reuploadQrBtn').onclick = function() {
        // Показываем форму загрузки
        showProfilePageUploadOnly(username, bar_name, userPhoto);
      };
    } else {
      ...
      document.getElementById('qrUploadForm').onsubmit = async function(e) {
        ...
        let msgDiv = document.getElementById('qrUploadMsg');
        if (!fileInput.files.length) {
          msgDiv.innerHTML = '<span style="color:#ff6b81;">Выберите файл</span>';
          return;
        }
        let formData = new FormData();
        formData.append('user_id', userId);
        formData.append('file', fileInput.files[0]);
        msgDiv.innerHTML = 'Загрузка...';
        let resp = await fetch(`${backend}/upload_qr`, {
          method: 'POST',
          body: formData
        });
        let data = await resp.json();
        if (data.ok) {
          msgDiv.innerHTML = '<span style="color:#19c37d;">QR-код успешно загружен!</span>';
          setTimeout(showProfilePage, 900);
        } else {
          msgDiv.innerHTML = '<span style="color:#ff6b81;">Ошибка: '+escapeHtml(data.error)+'</span>';
        }
      };
    }
  });
...
function showProfilePageUploadOnly(username, bar_name, userPhoto) {
  setPageTitle('Профиль');
  showBottomNav(true);
  let html = '';
  html += `<div style="display:flex;flex-direction:column;align-items:center;gap:18px;">`;
  ...
  html += `<div style="margin-top:28px;color:#888;font-size:1.05em;">Версия приложения: <b>v0.7</b></div>`;
  html += `</div>`;
  showPage(`<div id="profileBlock" class="beautiful-form" style="max-width:420px;text-align:center;">${html}</div>`);
  document.getElementById('qrUploadForm').onsubmit = async function(e) {
    e.preventDefault();
    let fileInput = document.getElementById('qrFileInput');
    let msgDiv = document.getElementById('qrUploadMsg');
...
navMap['nav-profile'] = showProfilePage;
